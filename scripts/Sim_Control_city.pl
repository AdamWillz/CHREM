...
SIMULATION: {
	...
	foreach my $core ($cores->{'low'}..$cores->{'high'}) {	#simulate the appropriate list (i.e. QC2 goes from 9 to 16)
		...
		system ("nohup ./Core_Sim_special.pl $core $set_name > $file &");	#call nohup of simulation program script and pass the argument $core so the program knows which set to simulate
	} 
