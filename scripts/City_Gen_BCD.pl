...
my @CHREMBulbs    :shared; # Array holding the keys for the CHREM bulb types
@CHREMBulbs = qw(Fluorescent Halogen Incandescent);
# --------------------------------------------------------------------
...
my $OccSTART = '../bcd/CREST/Occ_Lighting/occ_start_states.xml';
$occ_strt = XMLin($OccSTART);
print "Complete\n";

print "Reading in the light simulation parameters XML - ";
my $LIGHT = '../bcd/CREST/Occ_Lighting/lightsim_inputs.xml';
...
# --------------------------------------------------------------------
my $ColdFile = '../bcd/CREST/Appliance/Cold/Refrigerator_dist.xml';
$ColdApp->{'Refrigerator'}->{'dist'}=XMLin($ColdFile);
$ColdFile = '../bcd/CREST/Appliance/Cold/Refrigerator_eff.xml';
$ColdApp->{'Refrigerator'}->{'eff'}=XMLin($ColdFile);

$ColdFile = '../bcd/CREST/Appliance/Cold/Freezer_dist.xml';
$ColdApp->{'Freezer'}->{'dist'}=XMLin($ColdFile);
$ColdFile = '../bcd/CREST/Appliance/Cold/Freezer_eff.xml';
$ColdApp->{'Freezer'}->{'eff'}=XMLin($ColdFile);

# --------------------------------------------------------------------
# Load in general appliance data
# --------------------------------------------------------------------
my $AppFiles =  '../bcd/CREST/Appliance/Appliance_inputs.xml';
$App = XMLin($AppFiles);
...
my $ActStatpth = '../bcd/CREST/Appliance/activity_stats.csv';
...
MAIN: {
	sub main () {
        ...
        # -----------------------------------------------
        # Set the Calibration scalars for the BCD file generation
        # -----------------------------------------------
        my $LightCalib =$light_sim->{"_$region"}->{"_$hse_type"}->{'Calibration'};
        my $AppCalib = $App->{"_$region"}->{"_$hse_type"}->{'Calibration'};
        ...
        CREST_LOAD: foreach my $house_name (@houses_desired) {
            ...
            # --------------------------------------------------------------------
            my $NNdata;
            if (exists $NNinput->{'data'}->{"$house_name.HDF"}) {
                $NNdata = $NNinput->{'data'}->{"$house_name.HDF"};
...
# 									};
	...
	# 									};
			...
			BCD: {
                ...
                my @DHW_Draw;
                my $ThisBase = $App->{"_$region"}->{"_$hse_type"}->{'Baseload'}; # Constant baseload AL power [W]
                ...
                MATCH_DHW: {
                    ...
                    if($time_step>$MeasTstep) {
                        ...
                        if(!$Errr) {
                            $issues = set_issue("%s", $issues, 'DHW', 'Timestep averaging', "Could not average DHW data. Skipping", $coordinates);
                            next RECORD;
                        };
                ...
                SYNTH_AL: {
                    ...
                    # --------------------------------------------------------------------
                    my $IrrDir = "../bcd/CREST/Global_Horiz/$region";
                    opendir (DIR, $IrrDir) or die $!;
                    while (my $file = readdir(DIR)) {
                        if ($file =~ m/$loc/) {
                            my $irradiance = "../bcd/CREST/Global_Horiz/$region/$file";
...
# 				&replace ($hse_file->{'aim'}, '#WINDOW_CONTROL', 1, 1, "%s\n", "1 $zones->{'name->num'}->{'main_1'} 30 23 1 10");
			...
			AFN: {
				my $afn = {};
				our $AfnCount = 0; # Counter to be used with indexer below
				our @AfnCompInd; # Hold the indices of AFN components for windows
                undef(@AfnCompInd); # Reinitialize the array
				...
				# Write the control data for the AFN to operate windows. Check first to see if there are windows in the main and basement zones
				my $NumWinComp=scalar @AfnCompInd;
				my $kownt=0;
				# However unlikely, check to see if there is windows in the main and basement zones and define window controls accordingly
				if ($NumWinComp >= 1) {
					&insert ($hse_file->{'ctl'},'#END_AFN_FUNCTIONS_DATA',1, 0, 0, "%s \n%s \n%s \n%s \n", "* Mass Flow","no flow control description supplied","#NUM_AFN_FUNCTIONS number of afn functions", "$NumWinComp");
					# Cycle through the window components and add a control
					while ($kownt < $NumWinComp) { # Cycle over all window components and assign control
						# Check to see if an aperture is defined on that side
						&insert ($hse_file->{'ctl'}, '#END_AFN_FUNCTIONS_DATA', 1, 0, 0, "%s", &AFN_control($kownt,$zones->{'name->num'}->{"$AfnCompInd[$kownt][1]"}));
						$kownt++;	
					};
				};
			};
