...
sub collect_results_data {
	...
	FOLDER: foreach my $folder (@folders) {
        ...
        my $Actual_hse = $hse_name; 
        $Actual_hse =~ s/_[0-9]+//; 	 # Store the actual CSDDRD record name (in case of duplicates)
		...
		my $filename = $folder . "/$Actual_hse.bps";
		...
		$filename = $folder . "/$Actual_hse.cfg";
...
# 		print "The zone nums $main_bsmt_zone_nums\n\n";;
		...
		&organize_xml_log($folder . "/$Actual_hse", $sim_period, $zone_name_num, $province[0], $coordinates);
		...
		unless (grep(/$Actual_hse.xml$/, <$folder/*>)) {
		...
		my $results_hse = XMLin($folder . "/$Actual_hse.xml");
