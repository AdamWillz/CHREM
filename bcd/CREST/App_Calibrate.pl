...
our $hse_type;   
our $region;
our $Target;                 # Target average annual appliance consumption for region and hse_type [kWh/yr/hsehld]
...
SET_CREST: {
    ...
    REC: while ($CSDDRD = &one_data_line($LIST, $CSDDRD)) { # Each house in the CSDDRD record
        ...
        # --------------------------------------------------------------------
        # Store cold appliance stock for the house
        # --------------------------------------------------------------------
        ...
        foreach my $cold (@Cold_key) {
            $CREST->{$hse_name}->{'data'}=$NNdata;
        };
